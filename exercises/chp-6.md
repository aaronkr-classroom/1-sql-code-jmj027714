...
장점:
데이터 검색 속도 향상​
WHERE 절의 조건 처리 효율 증가​
정렬 및 그룹핑 성능 개선​

단점:
데이터 삽입, 수정, 삭제 시 인덱스 유지 비용 발생​
과도한 인덱스 생성 시 저장 공간 증가​
잘못된 인덱스 사용 시 오히려 성능 저하

**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**

바람직한 경우:
대량의 데이터에서 특정 조건으로 자주 검색할 때​
JOIN, ORDER BY, GROUP BY 절이 자주 사용될 때​
WHERE 절에 사용되는 컬럼에 인덱스를 생성할 때​

바람직하지 않은 경우:
데이터 변경(삽입, 수정, 삭제)이 빈번한 테이블​
소량의 데이터로 구성된 테이블​
자주 사용되지 않는 컬럼에 인덱스를 생성할 때
